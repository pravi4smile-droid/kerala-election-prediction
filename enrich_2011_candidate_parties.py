#!/usr/bin/env python3
"""Enrich data/2011/*.json candidate rows from Kerala Assembly 2011 result pages."""
...
SOURCE_URL = "http://www.keralaassembly.org/election/assembly_poll.php?no={no}&year=2011"
...
UDF_PARTIES = {
    "INC", "IUML", "ML", "KC", "KC(J)", "KEC", "KCM", "CMP", "SJ(D)", "SJD",
    "JDU", "UDF",
}
...
def enrich_file(path: str, source_rows: list[dict]) -> int:
    ...
    matched = 0
    used = set()
    for cand in data.get("candidates_2011", []):
        ...
        cand.update({
            ...
            "actual_votes_2011": row["votes"],
            "vote_pct_2011": row["percentage"],
            "source": "keralaassembly.org 2011 result page",
    ...
    data["public_source_2011"] = SOURCE_URL.format(no=data["const_no"])
